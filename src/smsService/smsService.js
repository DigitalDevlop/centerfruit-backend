...
const { loggerSMS,loggerDaraz,LoggerReloadSMS } = require('../logger/logger');
...
const sendAttemptMessage = async (mobile, messageTemplate,msgCategory) => {
    ...
    try {
        ...
        const messageState = response.data === messageResponse.messageResponse ? SMSStatus.DELIVERED : SMSStatus.FAILED;
        const smsLog = await createNewSMSLog(mobile, messageTemplate, messageState,msgCategory);
        console.log(`Message ${messageState.toLowerCase()}:`, smsLog);
        loggerSMS.info(`Message ${messageState.toLowerCase()} to ${mobile}: ${JSON.stringify(smsLog)}`);
...
const sendReloadWinningMessage = async (mobile,message,msgCategory) => {
    ...
    try {
        ...
        const smsLog = await createNewSMSLog(mobile, message, messageState,msgCategory);
        console.log(`Message ${messageState.toLowerCase()}:`, smsLog);
        LoggerReloadSMS.info(`Message ${messageState.toLowerCase()} to ${mobile}: ${JSON.stringify(smsLog)}`);

    } catch (error) {
        console.error('Error sending message:', error.response ? error.response.data : error.message);
        LoggerReloadSMS.error(`Error sending message to ${mobile}: ${error.response ? JSON.stringify(error.response.data) : error.message}`);
    }
};
